...
/**
 *
 * Represents a hypothetical (but possible) board state. Used to implement
 * Monte Carlo Tree Search Algorithm. Needs to know the hypothetical board state,
 * its parent node, the piece that is supposed to move, and the previous move made
...
public class Node{
	...
	*
	* @param board hypothetical board state
	* @param parentNode parent of current node, through which this current node can be accessed
	* @param piece piece type that is next to move
	* @param prevMove previous move made to get to this board state
	*/
	...
	*/
	public void buildChain(){
		...
		ArrayList<ArrayList<CoordinatePair>> list = null;
		...
		for (int i = 0; i < boardMoves.length; i++){
			...
			updatedBoard[CoordinatePair.decodeRow(boardMoves[i])][CoordinatePair.decodeCol(boardMoves[i])] = nextMovePiece;
	...
	/**
	*
	* Updates child nodes stemming from current node. Used only when
	* it is not possible to generate child nodes. Updates piece type
	* that is next to move.
	*/
	...
	*
	* @return returns true if there are possible child nodes from the current node, false otherwise
	*/
	...
	*/
	...
	/**
	*
	* Sets the current node as the parent to all nodes. Represents the real board state from
	* which all other nodes will stem.
	*
	*/
	...
	/**
	*
	* Updates number of simulations run and wins when running simulations through this node.
	* Used in backpropagation phase.
	*
	* @param win whether or not a simulation going through this node has resulted in a win
	*/
	...
	*
	* @param node node whose boardstate will be compared to calling objects'
	*
	...
	*/
	...
	/**
        * Compares node's board state and a board
        *
	* @param board board that will be compared to the board state within a node
	* 
	* @return whether or not the board states  node's and argument's are identical
	*/
	...
	*
	* @return returns true if this node has had any child nodes generated, returns false otherwise
	*/
	...
	/**
	*
	* Returns the children of this node (possible board states stemming from this board state)
	*
	* @return children of this node (possible board states stemming from this board state)
	*/
	...
	*/
	...
	/**
	* Returns the number of simlulations that went through this node
	*
	...
	*
	* @return string representation of the move made from the parent node to get to this node
	*/
	...
	*
	* @param piece a piece that a node has to make a move with
	*
